...
class RNNModel(object):
    ...
    def create_data_set(self, data_set):
        ...
        for i in range(len(data_set) - self.look_back - 30):
            ...
            data_y.append(list(data_set[i + self.look_back+8: i + self.look_back+9, 0]))
    ...
    def run(self):
        ...
        plt.plot(purchase_scaler.inverse_transform(trainPredictPlot))
        plt.plot(purchase_scaler.inverse_transform(testPredictPlot))
        plt.show()
        # print(numpy.array([datay]))
        # print(numpy.array(purchase_train_x[0]))
...
if __name__ == '__main__':
    #15
    initiation = RNNModel(look_back=60, epochs_purchase=3, epochs_redeem=1, batch_size=16, verbose=2, patience=50,
                          store_result=True)
    initiation.run()
