...
# define model
model = Sequential()
model.add(LSTM(200, activation='relu', input_shape=(n_steps_in, n_look_back)))
model.add(RepeatVector(n_steps_out))
model.add(LSTM(200, activation='relu', return_sequences=True))
model.add(TimeDistributed(Dense(30)))
model.compile(optimizer='adam', loss='mse')
# fit model
model.fit(X, y, epochs=430, verbose=0)
...
test_user.to_csv('test/test_lstm2.csv', encoding='utf-8', index=None, header=None)
